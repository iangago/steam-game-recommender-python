...
def get_game_details(appid, game_cache):
    #tries to load game from cache
    game = cache.get_cached_game(game_cache, appid)

    #if games is on cache returns its genre
    if game != None:
        return game['genres']
    ...
    cache.add_game_to_cache(game_cache, appid, game_data['name'], return_genres)
